/*
    ...
    20231211 Version 2.0.6    added 5mm and 8mm hex vent, added three m1s cases, updated sbc model framework,
                              fixed snap top protrusion placement and other fixes.
...
sata_punchout = false;
// gpio openings
gpio_opening = "none"; // [none,vent,open,block,punchout]
// cooling openings
cooling = "fan"; // [none,vents,fan,hex_5mm,hex_8mm,custom]
// exhaust vents
        ...
        union() {
            difference() {
                        ...
                        if(depth >= 75 && sbc_model != "visionfive2" && sbc_model != "visionfive2q" && sbc_model != "rock5b-v1.42" && sbc_model != "rock5bq-v1.42" && sbc_model != "rock5b-v1.3") {
                            ...
                            if(sbc_model == "visionfive2" || sbc_model == "visionfive2q" || sbc_model == "rock5b-v1.42" ||  sbc_model == "rock5bq-v1.42" || sbc_model == "rock5b-v1.3") {
                                ...
                                translate([width-wallthick-gap-wallthick-4+adjust,wallthick+gap+40,
                                        floorthick+3.4]) rotate([90,0,90]) cylinder(d=10, h=4, $fn=6);
                                translate([-adjust-gap,wallthick+gap+40,floorthick+3.4]) 
                                    rotate([90,0,90]) cylinder(d=10, h=4, $fn=6);
                            }
                        }
                ...
                if(case_design == "tray") {
                    ...
                    translate([-wallthick-gap-adjust,wallthick+gap+10,floorthick+3.4]) rotate([0,90,0]) 
                    ...
                    if(depth >= 75 && sbc_model != "visionfive2" && sbc_model != "visionfive2q" && sbc_model != "rock5b-v1.42" && sbc_model != "rock5bq-v1.42" && sbc_model != "rock5b-v1.3") {
                        ...
                        if(sbc_model == "visionfive2" || sbc_model == "visionfive2q" || sbc_model == "rock5b-v1.42" ||  sbc_model == "rock5bq-v1.42" || sbc_model == "rock5b-v1.3") {
                            ...
                            translate([width-3*(wallthick+gap)-adjust,wallthick+gap+40,
                                floorthick+3.4]) rotate([0,90,0]) cylinder(d=3, h=10+sidethick+(2*adjust));
...
module open_io() {
    ...
    for (i=[69:6:len(sbc_data[s[0]])-1]) {   
        ...
        if(side == "top" && cooling == "hex_5mm" && class == "heatsink" && type != "m1s_oem") {
                translate([loc_x+2,loc_y-27,case_z-(floorthick+adjust)-6]) 
                    vent_hex(15, 7, floorthick+(adjust*2)+12, 5, 1.5, "horizontal");
        }
        if(side == "top" && cooling == "hex_8mm" && class == "heatsink" && type != "m1s_oem") {
                translate([loc_x,loc_y-31,case_z-(floorthick+adjust)-6]) 
            ...
            || cooling == "custom") && class == "heatsink" && gpio_opening != "vent" && gpio_opening != "open" 
                && gpio_opening != "punchout") {
                    for(r=[loc_x+7:4:50+loc_x]) {
                        translate([r,depth-(2*wallthick)-adjust-3,bottom_height+2]) 
                           cube([2,wallthick+(2*adjust)+2.5,top_height-floorthick-4]);
                    }
        }
        ...
        if(side == "top" && class == "gpio" && type == "header_40" && rotation == 0) {
            ...
            if(gpio_opening == "block") {
                if(sbc_model == "m1s") {
                    translate([loc_x-27,depth-(2*wallthick)-adjust-gap-10,bottom_height+2])
                        cube([79,20,top_height]);
                }
                else {
                    translate([loc_x-2,depth-(2*wallthick)-adjust-gap-10,bottom_height+2])
                        cube([54,20,top_height]);
                }
            }
        ...
        if(side == "top" && class == "gpio" && type == "header_40" && rotation == 90) {
            if(gpio_opening == "vent") {
                for(r=[loc_y-2:4:50+loc_y]) {
                    translate([width-2*(wallthick+gap)-adjust,depth-r,bottom_height+2]) 
                        rotate([0,0,0]) cube([6,wallthick+(2*adjust),top_height-floorthick-4]);
                }
            }
            if(gpio_opening == "open") {
            ...
            if(gpio_opening == "block") {
                translate([loc_x+8,depth-13,bottom_height+2])
                    rotate([0,0,-90]) cube([54,wallthick+(2*adjust),top_height]);
            }
            if(gpio_opening == "punchout") {
        ...
        if(side == "top" && class == "gpio" && type == "encl_header_30") {
            ...
            if(gpio_opening == "block") {
                translate([loc_x-2,depth-(2*wallthick)-adjust-10,bottom_height+2])
                    cube([41,20,top_height]);
            }
            if(gpio_opening == "punchout") {
        ...
        if(side == "top" && class == "gpio" && type == "header_26") {
            if(gpio_opening == "vent") {
                for(r=[loc_x-2:4:39+loc_x]) {
                    translate([r,-wallthick-gap-adjust,bottom_height+2]) 
                        cube([2,wallthick+(2*adjust),top_height-floorthick-4]);
                }
            }
            if(gpio_opening == "open") {
                translate([loc_x-2,-wallthick-gap-adjust,bottom_height+3])
                    cube([41,wallthick+(2*adjust),top_height-floorthick-5]);
            }
            if(gpio_opening == "block") {
                translate([loc_x-2,-wallthick-gap-adjust-10,bottom_height+2])
                    cube([41,20,top_height]);
            }
            if(gpio_opening == "punchout") {
                translate([loc_x+1,-wallthick-adjust-gap,bottom_height+8])
                    rotate([-90,0,0]) punchout(34,11,2,wallthick+(2*adjust),3,"slot");
            }
        }
