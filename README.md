...
![Image](./assets/SBC_Case_Builder_Cases.gif)
...
![Image](./assets/SBC_Case_Builder_GUI.gif)
...
Both standard motherboard adapters and cases can be generated with SBC Case Builder and this tab controls the specific settings. Since it is not possible to know the motherboard thickness it is set to 2mm and the *standard motherboard thickness* setting is used to add or subtract thickness for an accurate representation of the user provided PCB.  The *rear io shield* setting enables the generation of an io shield for adapters and standard motherboard cases.  The setting *adapter sbc rotation* rotates the sbc for adapters.  Most of the adjustments in the *3D Case Adjustments* tab also work for motherboard adapters which allows for the SBC or other device to be repositioned on the adapter while providing the full use of the accessory subsystem. Any accessory entries in the sbc_case_builder_accessories.cfg file should reference the bottom face. The *bottom cover pattern* in the *Fan and Vent Openings* tab and the *SBC Bottom Standoff* tabs also apply to motherboard adapters.  There are predefined example cases and adapters available in the GUI for further reference.
